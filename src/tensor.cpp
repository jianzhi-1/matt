...
namespace matt {
    ...
    ): _storage(std::move(storage)), _shape(std::move(shape)), _strides(std::move(strides)), _offset(offset), _requires_grad(requires_grad){

    }

    size_t _size(const std::vector<size_t> shape){ // should this be &&?
        size_t cumulative_size = 1;
        for (auto it: shape) cumulative_size *= (*it);
    ...
    Tensor Tensor::from_data(const std::vector<float>& data, std::vector<size_t> shape){
        ...
        if (cumulative_size != data.shape()){
            throw std::runtime_error("Data size does not match shape."); // is there a more standard way of raising assertions, also with data.shape and cumulative_size
        }
    ...
    Tensor Tensor::arange(float start, float stop, float step){
        ...
        for (float v = start; v < stop; v += step) data.push_back(v);
        size_t n = data.size();
    ...
    float& Tensor::at(std::vector<size_t> indices){
        return (storage_ -> data())[flat_index(indices)]; // is it possible to get rid of these duplicates?
    }
    ...
    size_t Tensor::flat_index(const std::vector<size_t>& indices) const {
        if (indices.size() != shape_.size()) throw std::runtime_error("Index rank mismatch.");
