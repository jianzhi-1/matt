...
Tensor Zeros::initialize(std::vector<size_t> shape) const {
    return Tensor::zeros(shape);
}

Uniform::Uniform(float low, float high, uint32_t seed): low_(low), high_(high), seed_(seed){}

Tensor Uniform::initialize(std::vector<size_t> shape) const {
...
Tensor KaimingUniform::initialize(std::vector<size_t> shape) const {
    if (shape.size() < 2) throw std::runtime_error("KaimingUniform: shape must be at least 2D");
...
Tensor Normal::initialize(std::vector<size_t> shape) const {
    std::mt19937 rng(seed_);
